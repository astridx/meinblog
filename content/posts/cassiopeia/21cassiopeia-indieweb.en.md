...
Alright, at this point I will describe the steps I took to set up my Joomla website for the IndieWeb. The website [indiewebify.me](https://indiewebify.me/), was a great help in this.
...
You probably already have many profiles on the web. If you link them to your domain name using the microformat [`rel=me`](http://microformats.org/wiki/rel-me), it is easy to see that you are the same person on Google, Twitter, Github, Flickr, Facebook, email and that this person belongs to your domain name.
...
Wahrscheinlich hast du bereits viele Profile im Web. Wenn du diese mit dem Mikroformat [`rel=me`](http://microformats.org/wiki/rel-me) mit deinem Domänennamen verknüpfst, ist es leicht zu erkennen, dass du bei Google, Twitter, Github, Flickr, Facebook, E-Mail dieselbe Person bist und diese Person zu deiner Domänenname gehört.
...
> Would you like to test the login via e-mail address? Register with webmention.io via your website. You will receive an e-mail to the `rel="me"` e-mail address with which you authenticate yourself.
...
If you reply to something on another IndieWeb site or mention someone with an IndieWeb site, you can send a [webmention](http://webmention.org/). This allows a site visitor to the other site to see that it has been linked to your site. Sending webmentions allows you to post replies to other content on your own site and participate in cross-site conversations. Sounds complicated? Well, it's just like any other social network where you react to a post by commenting or liking it.
...
#### Add reply to your page

Posting [replies](https://indieweb.org/reply) to other people's posts is the next step after mentioning them with webmention.

Normally, a reply is a [note](https://indieweb.org/note) like any other, plus it links to the post it is replying to. If it is marked with `h-entry` and `rel=in-reply-to` and/or `class=u-in-reply-to`, the reply may appear as a comment on the original post.
...
Via Task Plugin I fetch webmentions from `https://webmention.io` once a day and save them in a file. For this, a token `https://webmention.io` of is required, which I insert per parameter in the settings for the plugin.
...
You do this, for example, by tweeting about your post and linking to your website. 

`Backfeed` describes the process of pulling the interactions of your POSSE copy to the original post. So if someone comments on a tweet with the link to your post, it will actually link back to your website as a webmention.
