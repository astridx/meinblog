...
The first part is React agnostic, but the second and third glue the GraphQL client (data layer) to React (view layer). It can be seen as an analog to the _redux_ and _react-redux_ or _apollo-client_ and _react-apollo_ libraries. The former is view layer agnostic, the latter is used to connect it to the view layer.
...
Necessary information is provided for the GraphQL client constructor to create a GitHub GraphQL client instance. In previous applications, you have seen how to obtain the [personal access token from GitHub to access their data](https://help.github.com/articles/creating-a-personal-access-token-for-the-command-line) and how to use it in a _.env_ file for environment variables, to make it securely accessible for the GraphQL client instantiation.
